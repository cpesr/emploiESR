...
```{r contexte_postes}
plot_emplois_long() + cpesr_cap() + ggtitle("Nombre de postes de Maîtres de conférences ouverts au concours par an") + coord_cartesian(clip = "off")
```
...
```{r contexte_ec}
p_contexte_ec.norm + cpesr_cap() + ggtitle("Evolution des effectifs enseignants-chercheurs titulaires", subtitle="Valeur 100 pour 2009")
```
...
p_contexte_etu.norm + cpesr_cap() + ggtitle("Evolution des effectifs étudiants dans le périmètre du MESRI", subtitle = "en valeur 100 pour 2009")
```
...
p_contexte_te.norm + cpesr_cap() + ggtitle("Evolution des taux d'encadrement", subtitle = "nombre d'enseignants-chercheurs titulaires pour 100 étudiants, en valeur 100 pour 2009")
```

### Evolution des effectifs étudiants et enseignants-chercheurs titulairs et taux d'encadrement

```{r p_contexte_ec.etu.te}
p_contexte_ec.etu.te + cpesr_cap() + ggtitle("Evolution des effectifs étudiants et EC titulaires, et du taux d'encadrement", subtitle = "valeur 100 pour 2009")
```

```{r p_contexte_ec.etu.te.disc}
p_contexte_ec.etu.te.disc  + cpesr_cap()  + ggtitle("Evolution des effectifs étudiants et EC titulaires, et des taux d'encadrement", subtitle = "valeur 100 pour 2009")
```
...
```{r contexte.evol.etu}
p_contexte.evol + cpesr_cap() + ggtitle("Evolutions réelles et nécessaires des effectifs enseignant-chercheur titulaire", subtitle="pour maintenant le taux d'encadrement de 2009")
```
...
```{r flux.col}
p1 + ggtitle("Effectifs aux différentes étapes de la sélection lors du recrutement des MCF", subtitle = "en 2019") + cpesr_cap()
```

```{r flux.col.discipline}
p2 + ggtitle("Effectifs aux différentes étapes de la sélection lors du recrutement des MCF", subtitle="en 2019") + cpesr_cap()
```
...
```{r flux.alluvial}
p3 + ggtitle("Effectifs aux différentes étapes de la sélection lors du recrutement des MCF", subtitle="en 2019") + cpesr_cap()
```

```{r flux.alluvial.discipline}
p4 + ggtitle("Effectifs aux différentes étapes de la sélection lors du recrutement des MCF", subtitle="en 2019") + cpesr_cap()
```
