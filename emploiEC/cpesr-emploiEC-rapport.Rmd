...
::: {.datapagecom data-latex=''}
## Nombre de postes MCF ouverts au concours

__Définition__ : _Nombre de postes de Maître de Conférences ouverts au concours 
...
plot_groupes <- function(groupe,gd) {
  ...
  p_flux_alluvial <- plot_flux_alluvial(périm="Groupe", périmid=groupe, sizemult = 0.5) + 
    theme_rapport_small() +
    theme(legend.position = "none", strip.background = element_blank(), strip.text = element_blank())
  p_séries <- plot_series(séries.MCF, séries.MCF.lab, périm="Groupe", périmid=groupe, norm=FALSE) +  
    theme(legend.position = "none",  strip.background = element_blank(), strip.text = element_blank())
...
cnu.gd <- cnu.sections %>% select(GrandeDisciplineCNU.ID,GroupeCNU.ID,GroupeCNU) %>% unique() %>% arrange(as.numeric(GroupeCNU.ID))
