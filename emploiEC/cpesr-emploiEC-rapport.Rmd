...
knitr::opts_chunk$set(dev = "cairo_pdf", pdf.options(encoding = "CP1250"))
...
Les effectifs étudiants en Pharmacie à partir de 2021 sont sujets à caution. Un changement de périmètre des données explique probablement la hausse constatée sur ces graphiques.

:::

::: {.datapageviz data-latex=""}
```{r contexte.ec.etu}
l <- cowplot::get_legend(p_contexte_ec +
...
Les effectifs étudiants en Pharmacie à partir de 2021 sont sujets à caution. Un changement de périmètre des données explique probablement la baisse constatée sur ces graphiques.
...
plot_groupes <- function(groupe,gd) {
  ...
  p_metriques <- plot_all_groupe(groupe, gd,
                   ...
                   norm=FALSE,
                   sizemult = 0.5,
                   zeroing = TRUE)  + theme_rapport_small()

  p_legend1 <- get_legend(p_séries + theme(legend.position = "right", legend.direction = "horizontal"))
  p_legend2 <- get_legend(p_metriques + theme(legend.position = "right", legend.direction = "horizontal", legend.spacing.x = unit(0.3, "cm")))
...
for (s in 1:12) { 
  ...
  sections <- cnu.sections %>% filter(groupe == GroupeCNU.ID) %>% pull(SectionCNU.ID) %>% range() %>% paste(collapse = " à ")
...
cat(emploisEC %>% 
  filter(Périmètre == "Groupe", Périmètre.ID == groupe, Données == "Non consolidées") %>%
...
cat(emploisEC %>% 
  filter(Périmètre == "Section", Périmètre.ID == section, Données == "Non consolidées") %>%
  select(Année, `Postes MCF`=Concours.Postes.MCF) %>%
  kableExtra::kable("latex", booktabs = T)
) 
