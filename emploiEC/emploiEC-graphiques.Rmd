...
```{r ensemble.mcf, fig.width=8}
postes %>%
  filter(Corps == "MCF") %>%
  group_by(Année,Donnée,Corps) %>%
  summarise(Postes = sum(Postes, na.rm = TRUE)) %>%
  ggplot(aes(x=Année, y=Postes)) + 
    geom_smooth(se=F) + 
    geom_point(aes(color=Donnée),size=1) + 
    expand_limits(y=0) +
    scale_color_brewer(palette = "Paired", direction = -1) +
    theme_cpesr_cap() + 
    theme(plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
    labs(title="Postes au concours de Maître de Conférences", y ="Postes publiés", x = "Année") 
```

## Concours de Professeur des universités


```{r ensemble.pr, fig.width=8}
postes %>%
  filter(Corps == "PR") %>%
  group_by(Année,Donnée,Corps) %>%
  summarise(Postes = sum(Postes, na.rm = TRUE)) %>%
  ggplot(aes(x=Année, y=Postes)) + 
    ...
    theme_cpesr_cap() + 
    theme(plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
    labs(title="Postes au concours de Professeur des universités", y ="Postes publiés", x = "Année") 
```

```{r funs}
plot_section <- function(section, sectionid, corps) {
  postes %>% 
    filter(SectionCNU.ID==as.numeric(sectionid), Corps == corps) %>% 
    ggplot(aes(x=Année, y=Postes)) + 
      geom_smooth(se=F) + 
      geom_point(aes(color=Donnée),size=1) + 
      expand_limits(y=0) +
      ...
      theme_cpesr_cap() + 
      theme(plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm")) +
      labs(title=paste0("Concours ",corps," section CNU ", sectionid), subtitle=section, y ="Postes publiés", x = "Année") 
}
```

```{r evolution.postes, fig.width=8, fig.asp=9/16, results='asis'}
...
for(s in 1:nrow(sec)) {
  ...
  if (sectionid > 90) break
  
  cat(paste0("\n\n## MCF Section #CNU", sectionid," ", section,"\n"))
  print(plot_section(section, sectionid, "MCF"))
  
  cat(paste0("\n\n## PR Section #CNU", sectionid," ", section,"\n"))
  print(plot_section(section, sectionid, "PR"))  
}
```