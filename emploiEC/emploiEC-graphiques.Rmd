...
emploisEC %>% 
  ...
  mutate(
    ...
    `Candidat⋅es MCF` = Concours.Candidats.MCF,
  ...
  pivot_longer(c(`Postes MCF`,`Candidat⋅es MCF`,`Postes non pourvus`)) %>%
  mutate(name = factor(name, levels=c("Postes MCF","Candidat⋅es MCF","Postes non pourvus"))) %>%
