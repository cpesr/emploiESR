...
```{r load.etu, results='asis', eval=FALSE}
etuold <- read.csv2("data/fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement-hcp.csv") %>%
...
etunew <- read.csv2("data/fr-esr-atlas_regional-effectifs-d-etudiants-inscrits-detail_etablissements.csv") %>%
  filter(catégorie.d.établissement %in% c("Universités","Autre établissements d'enseignement universitaire"), secteur.d.établissement == "Public") %>% 
  rename(Année = Rentrée.universitaire) %>%
  summarise(Effectif.ETU = sum(nombre.total.d.étudiants.inscrits.hors.doubles.inscriptions.université.CPGE), .by=Année) %>%
  mutate(
    Périmètre = "Ensemble",
    Périmètre.ID = "Ensemble")

etu <- bind_rows(
  etuold %>% filter(Périmètre != "Ensemble"),
  etuold %>% filter(Périmètre == "Ensemble", Année < min(etunew$Année)),
  etunew) %>%
...
```{r load.etu.stat, results='asis'}
#  [98] "Grande.discipline...Droit..sciences.économiques..AES"                                                        
#  [99] "Grande.discipline...Lettres..langues.et.sciences.humaines"                                                   
# [100] "Grande.discipline...Théologie"                                                                               
# [101] "Grande.discipline...Sciences.et.sciences.de.l.ingénieur"                                                     
# [102] "Grande.discipline...STAPS"                                                                                   
# [103] "Grande.discipline...Santé"                                                                                   
# [104] "Grande.discipline...Interdisciplinaire"                                                                      
# [105] "Grande.discipline...Non.défini"      
etu <- read.csv2("data/fr-esr-statistiques-sur-les-effectifs-d-etudiants-inscrits-par-etablissement-hcp.csv", dec='.')  %>%
  filter(Attention == "Sans double compte des établissements-composantes pour les EPE") %>%
  summarise(
    Ensemble = sum(Nombre.d.étudiants.inscrits..inscriptions.principales..hors.doubles.inscriptions.CPGE),
    DEG = sum(Grande.discipline...Droit..sciences.économiques..AES, na.rm=TRUE),
    LLASHS = sum(Grande.discipline...Lettres..langues.et.sciences.humaines, na.rm=TRUE),
    Pharma = sum(Secteur.disciplinaire...Pharmacie, na.rm=TRUE),
    ST = sum(Grande.discipline...Sciences.et.sciences.de.l.ingénieur, na.rm=TRUE),
    Santé = sum(Grande.discipline...Santé, na.rm=TRUE),
    Théologie = sum(Grande.discipline...Théologie, na.rm=TRUE),
    .by = annee
  ) %>%
  rename(Année = annee) %>%
  pivot_longer(-Année, names_to = "Périmètre.ID", values_to = "Effectif.ETU") %>%
  mutate(Périmètre=ifelse(Périmètre.ID == "Ensemble", "Ensemble","Grande discipline"))
...
odyssee <- bind_rows(
    read.csv(paste0(dir,"odyssee_20250304.csv")),
    read.csv(paste0(dir,"odyssee_20260303.csv"))) %>% 
  rename(SectionCNU.ID = section, Corps = corps) %>%
  left_join(cnu.sections) %>%
  mutate(Année = as.integer(str_sub(dateOuvertureCandidatures,1,4)))

odyssee %>%
  filter(dateScraping != max(dateScraping), .by=Année) %>%
  filter(Corps %in% c("PR","MCF")) %>%
  spoiler_table("MCF PR supprimés", trim=Inf)
...
concours <- bind_rows(
  ...
  cnu.fd %>%
    mutate(Source = "CNU", Données = "Consolidées"),
  galaxie %>%
    mutate(Source = "Galaxie", Données = "Non consolidées") %>%
    filter(Année > max(cnu.fd$Année)),
  odyssee %>%
    mutate(Source = "Odyssée", Données = "Non consolidées") %>%
...
emploisEC <- cnu.périmètres %>%
  right_join(concours) %>%
  left_join(ens) %>%
  left_join(etu %>% filter(Année %in% unique(ens$Année))) %>%
  mutate(
  ...
  #fill(Effectif.MCF,.direction = "down") %>%
  #fill(Effectif.PR,.direction = "down") %>%
  mutate(
    ...
    kpi.MCF.TauxPostesNonPourvus = (Concours.Postes.MCF - Concours.Recrutés.MCF) / Concours.Postes.MCF,
