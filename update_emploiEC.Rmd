...
cnu.sections %>% spoiler_table("cnu.section")
```
...
cnu.périmètres %>% spoiler_table("cnu.périmètres")
```
...
dgrh <- bind_rows(
  dgrh.raw %>%
    ...
    summarise(across(c(Concours.Postes.MCF, Concours.Recrutés.MCF),
                       ~ sum(.x,na.rm=TRUE))),
  dgrh.raw %>%
...
emploisEC %>% spoiler_table("emploisEC")
```
